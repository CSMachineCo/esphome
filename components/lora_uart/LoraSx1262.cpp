...
bool LoraSx1262::begin(int8_t pin_MISO, int8_t pin_MOSI, int8_t pin_NSS, int8_t pin_SCK, int8_t pin_RESET, int8_t pin_DIO1) {
  ...
  digitalWrite(_pin_NSS, 1);  //High = inactive
  pinMode(_pin_NSS,OUTPUT);
  digitalWrite(_pin_NSS, 1);  //High = inactive

  digitalWrite(_pin_RESET, 1);  //High = inactive
  pinMode(_pin_RESET,OUTPUT);
  
  pinMode(  _pin_DIO1, INPUT);  //Radio interrupt pin.  Goes high when we receive a packet

  //Hardware reset the radio by toggling the reset pin
  digitalWrite(_pin_RESET, 0); delay(100);
  digitalWrite(_pin_RESET, 1); delay(100);
  digitalWrite(_pin_RESET, 0); delay(100);
  digitalWrite(_pin_RESET, 1); delay(100);
...
bool LoraSx1262::sanityCheck() {
  ...
  digitalWrite(_pin_NSS, 0);  //CS Low = Enabled
  ...
  digitalWrite(_pin_NSS, 1);  //CS High = Disabled
...
uint8_t LoraSx1262::transmit(byte *data, int dataLen) {
  ...
  if(waitForRadioCommandCompletion(this->transmitTimeout)) //Wait for tx to complete, with a timeout so we don't wait forever
  {
    ...
    spiBuff[1] = 0x00;          //Dummy byte, status will overwrite this byte
    spiBuff[2] = 0x00;          //Dummy byte, status will overwrite this byte
    spiBuff[3] = 0x00;          //Dummy byte, status will overwrite this byte
    SPI.transfer(spiBuff,4);
    digitalWrite(_pin_NSS,1); //Disable radio chip-select
    return spiBuff[3]; 
    */
...
int LoraSx1262::lora_receive_blocking(byte *buff, int buffMaxLen, uint32_t timeout) {
  ...
  while (digitalRead(  _pin_DIO1) == false) {
